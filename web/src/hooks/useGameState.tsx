...
export default function useGameState(
  ...
  board.BoardState,
  (ids: number[]) => void,
  (card: board.FaceUpCardState) => void,
  boolean
] {
  ...
  const [data, setData] = useState<
    { card: card.CardData; id: number } | undefined
  >();
  ...
  useEffect(() => {
    (async () => {
      if (boardState.nextPhase === "play") {
        ...
        if (data?.id === id) {
          if (otherData?.id === otherId) {
            ...
            card
              .getCardData(otherId)
              .then((cardData) =>
                setOtherData({ card: cardData, id: otherId })
              );
          }
        } else {
          card
            .getCardData(id)
            .then((cardData) => setData({ card: cardData, id }));
        }
        ...
        await onUpdate();
        setBoardState((state) =>
          board.drawPhase(state, randInt.current!, isFirst)
        );
      }
    })();
