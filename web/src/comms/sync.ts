...
export function playMove(state: SyncState, move: any): SyncState {
  ...
  return markSent(
    send({ ...state, data: { ...state.data, move: saltedData } }, data)
  );
}
...
function markSent(state: SyncState): SyncState {
  return handleTurnStatusUpdate({
    ...state,
    status: { ...state.status, sent: true },
  });
}
...
function markReceived(state: SyncState): SyncState {
  return handleTurnStatusUpdate({
    ...state,
    status: { ...state.status, received: true },
  });
}
  ...
  if (state.status.sent && state.status.received) {
    return handleTurnUpdate({
      ...
      turn: {
        ...
        num:
          state.turn.phase === "reveal" ? state.turn.num + 1 : state.turn.num,
      },
    });
  } else {
    return state;
...
function handleTurnUpdate(state: SyncState): SyncState {
  if (state.turn.phase === "reveal" && state.data.move) {
    const data: MoveReveal = {
      key: "reveal",
      turn: state.turn,
      move: state.data.move,
    };
    return markSent(send(state, data));
  } else if (
    state.turn.phase === "setup" &&
    state.data.move &&
    state.data.otherMove &&
    state.data.otherHash
  ) {
...
function onSetup(state: SyncState, data: MoveSetup): SyncState {
  ...
  return markReceived({
    ...state,
    data: { ...state.data, otherHash: data.hash },
  });
}
...
function onReveal(state: SyncState, data: MoveReveal): SyncState {
  ...
  return markReceived({
    ...state,
    data: { ...state.data, otherMove: data.move },
  });
}
